...
1. 按项目既有结构、业务 workflow 或用例设计落点，不把所有流程塞进一个大文件。
2. 节点实现、状态定义、边条件、外部适配器和测试应职责清晰；具体目录名以项目约定为准。
3. 状态转移复杂时可使用 State；多阶段处理可使用 Pipeline；可重放动作可使用 Command。
4. 外部系统回调、队列、调度器和供应商 SDK 用 Adapter 隔离。
5. 业务源码文件默认不超过 800 行；超过时按业务流程、节点类型或外部适配器拆分。
...
- 长任务必须有超时、取消或恢复策略。
- 文件大小没有超过 800 行；超过时已按 workflow 边界拆分。
