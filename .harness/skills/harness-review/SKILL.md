...
- 升级 `.harness/agents/`、`.harness/skills/`、`.harness/workflows/` 或 `.harness/rules/`。
...
4. `.harness/agents/*.md`
5. `.harness/skills/*/SKILL.md`
6. `.harness/workflows/*.md`
7. `README.md`、`README.md.jinja`、`Makefile` 或 `Makefile.jinja`
8. 模板项目还要读取 `copier.yml`
...
3. Agent 元数据完整：`agent_type`、`model`、`reasoning_effort`、`specialization`。
4. Skill frontmatter 完整：`name`、`description`，且 description 能准确触发。
5. Skill 内容包含适用场景、必须读取、流程、质量门禁、验证和输出格式。
6. 工作流引用的 agent 和 skill 都存在。
7. 项目类型、包管理器、命令示例和脚本分支一致。
8. 文档没有把业务事实写进 `.harness/` 或把 AI 工作规则写进 `docs/`。
9. `make check` 或等价验证可达。
10. 安全、测试、审查规则能覆盖高风险改动。
